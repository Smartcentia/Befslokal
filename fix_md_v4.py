...
def fix_all(content):
    ...
    for i in range(len(fixed_lines)):
        ...
        if curr_strip.startswith('```'):
            if not in_pass2_fence:
                in_pass2_fence = True
                # Start of fence: don't add blank line AFTER (it would be inside)
            else:
                in_pass2_fence = False
                # End of fence: add blank line AFTER
                if i < len(fixed_lines) - 1 and fixed_lines[i+1].strip() != '':
                    final_lines.append('')
            continue
