...
* **Handlinger:** "Kan du opprette et innspill på at ventilasjonen er dårlig på kontoret i Bodø?" eller "Send en e-post til vaktmester om feil."

### Utføre handlinger (Handlinger på dine vegne)

KI Kollega kan nå **utføre handlinger** i systemet for deg, som for eksempel å opprette innspill (oppgave i oppgavestyring) eller sende beskjeder.
...
### To husleie-kilder – hva er forskjellen?

BEFS har to separate husleiekilder som **ikke** må blandes:

| Kilde | Felt | Betydning | Typisk bruk |
|-------|------|-----------|-------------|
| **Avtalefestet leie** | `contract_rent_nok` | Husleien som er spesifisert i leieavtalen/kontrakten. Importert fra «Oversikt bygg og eiendom»-CSV. ~181 eiendommer, ~319 MNOK. | Kontraktsanalyse, budsjettgrunnlag, kontraktsforpliktelser |
| **Faktisk husleie (GL 2025)** | `gl_rent_2025` | Faktisk bokført husleie fra Agresso-regnskap 2025 (SRS-kategori Lokaler). ~145 eiendommer, ~430 MNOK. | Regnskap, avviksanalyse, faktisk kostnadsbilde |

Differansen mellom de to kildene skyldes blant annet:
- **Periodisering:** Kontrakter kan ha endret seg i løpet av 2025
- **Omposteringer:** GL inneholder korrigeringer og omposteringer
- **Dekning:** Ikke alle eiendommer har begge kilder

KI-Kollega skiller automatisk mellom de to kildene når du spør om husleie. Spesifiser «avtalefestet» eller «bokført» hvis du vil ha én bestemt kilde.
...
**Sti:** `/jira` (INTEGRASJONER → Innspill).

Her kan du **opprette innspill** (oppgaver i det tilkoblede oppgavestyringssystemet) direkte fra BEFS uten å gå via KI-Kollega. Standard oppsett refererer til prosjektet **KAN (BEFS)** – avhengig av miljøkonfigurasjon. Vanlige issuetyper inkluderer **Epic** og **Oppgave**. Du kan legge til **etiketter** for bedre sporbarhet. Etter opprettelse åpnes saken ofte i ny fane.
...
**Q: Hvor oppretter jeg innspill uten å bruke chat?**
A: Gå til **INTEGRASJONER → Innspill** (`/jira`) og fyll ut skjemaet der.
