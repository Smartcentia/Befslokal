...
Denne rapporten viser at eiendommer (og avdelinger via mapping) har fått husleie fra Innkjøpsanalyse-CSV. Data lagres i `property_husleie_csv` og `total_kost_per_region_{year}.json`.

**Oppdater rapport:** Kjør `railway run python scripts/rapport_innkjøpsanalyse_husleie.py --year 2025` fra backend-mappen. Output lagres i `backend/data/rapport_innkjøpsanalyse_husleie_2025.md`.
...
Avdelinger (f.eks. «Stange 1», «Avdeling Sundstedtråkka») mappes til eiendom via `backend/data/eiendom_avdeling_mapping.json`. Når radetikett i Innkjøpsanalyse-CSV matcher et institusjonsnavn/avdelingsnavn i mappingen, lagres kostnaden under tilhørende eiendom i `property_husleie_csv`. 110 avdelinger er mappet.
