...
def parse_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    # 12.txt Format: 2 Region Øst 204416 ...
    ...
    # 12.txt seems to have property IDs around index 40-60
    # Try finding 7 digits
    prop_id_match = re.search(r'\b(\d{7})\b', line)
    property_id = None
    if prop_id_match:
        # verify it's not the center id
    ...
    # Logic: Text between property ID (if exits) and bureau? or center ID and bureau?
    # 12.txt example: ... 2050152 Kroerveien 9, 1540 Vestby Barnevernsinstitusjoner ...
    
    property_key = ""
    if property_id:
        pid_pos = line.find(property_id)
        pid_end = pid_pos + len(property_id)
        bureau_pos = line.find(bureau_found, pid_end)
        if bureau_pos > pid_end:
            property_key = line[pid_end:bureau_pos].strip()
            # If empty (weird), try fallback
        else:
             # Bureau might actally appear BEFORE property ID in some weird cases?
...
def get_exclusion_set():
    # Load 06.txt and 09.txt to prevent cross-contamination
    exclusion_files = ['docs/06.txt', 'docs/09.txt']
    seen_lines = set()
    ...
    for fpath in exclusion_files:
        if os.path.exists(fpath):
            with open(fpath, 'r') as f:
                for line in f:
                    s = line.strip()
                    if s: seen_lines.add(s)
    
    print(f"Loaded {len(seen_lines)} unique lines to exclude (from {', '.join(exclusion_files)})")
    return seen_lines
...
async def update_database(file_path: str):
    ...
    print(f"Skipped Cross-File Duplicates: {skipped_crosscheck}")
    ...
    async with SessionLocal() as db:
        ...
        for raw_key, transactions in by_property.items():
            ...
            # Logic: If we have a property ID, prioritizing matching that
            ...
            for p in all_properties:
                ...
                if pid:
                    # Check text fields for ID
                    if pid in p.name or (p.address and pid in p.address):
                        score = 1.0 # Perfect match
                    # Check JSON (if we have stored IDs previously)
                    # Not strictly implemented yet, but ID in name is common pattern
        ...
        await db.commit()
        print(f"Import Summary for 12.txt (Region Øst Part 2):")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python import_enrichment_12.py <file_path>")
    else:
