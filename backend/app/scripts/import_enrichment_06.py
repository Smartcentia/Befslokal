...
BUREAUS = [
    ...
    "beredskapshjem",
    "miljøavdeling",
    "avd. 1 foreldre og barn",
    "MST-team",
    "Multisystemisk terapi-team"
]
...
def parse_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    # Match ID (6 digits usually for cost center)
    ...
    # Check for a property ID (7 digits starting with 205 or similar)
    prop_id_match = re.search(r'\s(205\d{4})\s', line)
    ...
    for cat in sorted(EXPENSE_CATEGORIES, key=len, reverse=True):
        if cat in rest_for_cat:
            category_found = cat
            # Provider is often after category or before it
            # In 06.txt it seems to be [Category] [Provider]
            cat_pos = rest_for_cat.find(cat)
            provider = rest_for_cat[cat_pos + len(cat):].strip()
            if not provider:
                # Try before if empty
                provider = rest_for_cat[:cat_pos].split()[-1] if rest_for_cat[:cat_pos].strip() else "Ukjent"
            break

    # Property description
    # If we have property_id, use that as key, otherwise use the part between center_id and bureau
    ...
    if property_id:
        # Include property_id in the key for better matching if it's there
...
async def update_database(file_path: str):
    ...
    async with SessionLocal() as db:
        ...
        for raw_key, transactions in by_property.items():
            ...
            # Extract ID if present for direct match check
            ...
            for p in all_properties:
                score = 0.0
                # Direct ID match is strongest
                if pid and pid in (p.external_data or {}).get('property_id', ''):
                    score = 1.0
                else:
                    score = similar(clean_key, p.name.lower())
                    addr_score = similar(clean_key, (p.address or "").lower())
            ...
            if best_score > 0.60: # Region Øst might be slightly more varied
        ...
        await db.commit()
        print(f"Import Summary for 06.txt:")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python import_enrichment_06.py <file_path>")
    else:
