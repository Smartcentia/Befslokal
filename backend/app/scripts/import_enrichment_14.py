...
def parse_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    # 14.txt: 3 Region Sør ... (Region Sør, not Øst)
...
def get_exclusion_set():
    # Load 13.txt just in case (we saw 5 duplicates)
    exclusion_files = ['docs/13.txt'] 
...
async def update_database(file_path: str):
    ...
    async with SessionLocal() as db:
        ...
        await db.commit()
        print(f"Import Summary for 14.txt (Region Sør Part 2):")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python import_enrichment_14.py <file_path>")
    else:
