...
def parse_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    # Example: 5 Region Midt-Norge 513904 ...
    # Support Region Name with dashes
    ...
    for b in sorted_bureaus:
         # Need to be careful with space before bureau to avoid mid-word matches
        if f" {b}" in line:
            bureau_found = b
            break
            
    if not bureau_found:
        # Fallback to simple containment if space prefix fails
        for b in sorted_bureaus:
            if b in line:
                bureau_found = b
                break
    ...
    async with SessionLocal() as db:
        ...
        for raw_key, transactions in by_property.items():
            ...
            for p in all_properties:
                ...
                # Check for substring match as well
                if clean_key in p.name.lower() or (p.address and clean_key in p.address.lower()):
        ...
        await db.commit()
        print(f"Import Summary for 04.txt:")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python import_enrichment_04.py <file_path>")
    else:
