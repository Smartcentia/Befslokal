...
def parse_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    # 13.txt Format seems similar to 12.txt: 2 Region Øst 236103 ...
    ...
    # If bureau appears before ID (rare but possible), this logic breaks. Assumes standard order.
    
    if bureau_pos > start_pos:
        property_key = line[start_pos:bureau_pos].strip()
    
    # Clean up key if empty or just commas
    if not property_key or property_key == ",":
        # Fallback? sometimes address is embedded differently.
        pass
...
def get_exclusion_set():
    # Load 06.txt to prevent cross-contamination (Region Øst file)
    # Also 12.txt if we want? But 12 and 13 don't overlap based on previous check.
    # But 06 and 13 do overlap.
    exclusion_files = ['docs/06.txt', 'docs/09.txt', 'docs/12.txt'] 
    
    # Although previous check only showed overlap with 06.txt, it's safer to include all Region Øst files.
...
async def update_database(file_path: str):
    ...
    async with SessionLocal() as db:
        ...
        await db.commit()
        print(f"Import Summary for 13.txt (Region Øst Part 3):")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python import_enrichment_13.py <file_path>")
    else:
