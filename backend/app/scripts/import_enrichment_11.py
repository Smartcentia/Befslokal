...
# Import related models to populate registry FIRST
...
# Import Property LAST to ensure relationships can be resolved
...
async def update_database(file_path: str):
    ...
    if duplicates:
        print(f"WARNING: Found {len(duplicates)} duplicate lines in input file.")
        print("Duplicate examples:")
        for d in duplicates[:5]:
            print(f"  - {d}")
        print(f"Proceeding with {len(unique_lines)} unique transactions (filtered from {len(lines)} total lines).")
    else:
    ...
    print(f"Parsed {len(parsed_items)} valid transactions.")
    ...
    async with SessionLocal() as db:
        ...
        for raw_key, transactions in by_property.items():
            ...
            # Remove leading numeric codes if present (e.g. 55080)
            ...
            if best_score > 0.65:
                # print(f"Match: '{raw_key}' -> '{best_prop.name}' (Score: {best_score:.2f})")
                ...
                updated_count += 1
            # else:
            #     print(f"No match for: '{raw_key}' (Best score: {best_score:.2f})")
                
        await db.commit()
        print(f"Import Summary for 11.txt (Region Vest):")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python import_enrichment_11.py <file_path>")
    else:
