...
def parse_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    # 17.txt Format logic matches 14/13 likely
...
def get_exclusion_set():
    # Load 13.txt and 14.txt (based on analysis)
    exclusion_files = ['docs/13.txt', 'docs/14.txt']
...
async def update_database(file_path: str):
    ...
    async with SessionLocal() as db:
        ...
        await db.commit()
        print(f"Import Summary for 17.txt (Region Sør Part 3):")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python import_enrichment_17.py <file_path>")
    else:
