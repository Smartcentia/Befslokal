...
def parse_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    # 15.txt Format logic matches 14/13 likely
...
def get_exclusion_set():
    # Load 11.txt and 08.txt (based on analysis)
    exclusion_files = ['docs/11.txt', 'docs/08.txt']
...
async def update_database(file_path: str):
    ...
    async with SessionLocal() as db:
        ...
        await db.commit()
        print(f"Import Summary for 15.txt (Region Vest Part 2):")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python import_enrichment_15.py <file_path>")
    else:
