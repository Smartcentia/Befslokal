...
def parse_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    # Example: 6 Region Nord 624500 ...
    match = re.search(r'Region\s+\w+\s+(\d{5,6})\s+', line)
    if not match:
        return None
    ...
    # Property name is between ID and Bureau
    id_end = line.find(center_id) + len(center_id)
    bureau_start = line.find(bureau_found)
    
    property_name = line[id_end:bureau_start].strip()
...
async def update_database(file_path: str):
    ...
    async with SessionLocal() as db:
        ...
        for raw_name, transactions in by_property.items():
            ...
            for p in all_properties:
                score = similar(clean_name, p.name.lower())
                addr_score = similar(clean_name, (p.address or "").lower())
            ...
            if best_score < 0.7:
                 for p in all_properties:
                     if clean_name in p.name.lower() or (p.address and clean_name in p.address.lower()):
                         best_prop = p
                         best_score = 0.9 # High confidence for substring
                         break

            if best_score > 0.7:
                matches_found.append(best_prop.property_id)
